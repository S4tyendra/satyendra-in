...
const handleButtonClick = () => {
  ...
  if (activeItem.value.link.startsWith('http')) {
    ...
    router.push(activeItem.value.link);
  }
};
...
onMounted(() => {
  ...
  try {
    infiniteMenu = new InfiniteGridMenu(
      ...
      menu => menu.run(),
      props.scale // Pass scale here
    );

    // Initial Resize
    infiniteMenu.resize();

    // Use ResizeObserver for robust layout adaptation
...
<template>
  <div class="relative w-full h-full">
    ...
    <template v-if="activeItem">
      <div class="absolute inset-0 pointer-events-none flex flex-col justify-between p-8 md:p-12 z-10">
        <!-- Header / Top Area (Empty for now seems fine, or we can move things here) -->
        <div></div>

        <!-- Bottom Area: Title Left, Description Right -->
        <div class="flex flex-col md:flex-row justify-between items-end gap-6 w-full mb-12">

          <!-- Title -->
          <h2 :class="[
            'font-black text-4xl md:text-6xl text-white transition-all duration-500 ease-in-out leading-tight',
            isMoving
          ...
          <!-- Description -->
          <p :class="[
            'text-sm md:text-base text-zinc-300 transition-all ease-in-out max-w-xs md:max-w-md text-right',
            isMoving ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0 duration-500 delay-75'
          ]">
      ...
      <div @click="handleButtonClick" :class="[
        'absolute left-1/2 bottom-8 z-20 grid place-items-center bg-white text-black rounded-full cursor-pointer hover:scale-110 transition-all ease-in-out shadow-[0_0_20px_rgba(255,255,255,0.3)]',
        isMoving ? 'pointer-events-none opacity-0 scale-50' : 'opacity-100 pointer-events-auto scale-100'
      ]" :style="{
        width: '64px',
        height: '64px',
